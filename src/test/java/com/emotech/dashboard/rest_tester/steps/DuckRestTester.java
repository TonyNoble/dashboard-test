...
public class DuckRestTester extends AbstractRestTester {
  ...
  public void checkZeroDucks() {
	  this.setPath( "duck" ) ;
	  this.getUrl() ;
	  while( (int)this.getRestResponse().path( "duckCount" ) > 0 && this.getRestResponseStatus() == 200 ) {
		  this.deleteUrl() ;
  ...
  public void checkCurrentDuckCountIncremented() {
	  this.setPath( "duck" ) ;
	  this.getUrl() ;
	  Assert.assertTrue( (int)this.getRestResponse().path( "duckCount" ) > this.duckCount ) ;
  }
